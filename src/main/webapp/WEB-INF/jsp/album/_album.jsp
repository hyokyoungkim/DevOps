...
<%
    ...
    if (view == null) {  //로그인이 되었다면
        ...
        if (    pageId.indexOf("getAlbumList.do") != -1
            ||  pageId.indexOf("getAlbum.do") != -1
            ||  pageId.indexOf("getContentList.do") != -1
            ||  pageId.indexOf("getContent.do") != -1
            ||  pageId.indexOf("getCommentList.do") != -1          
            
            //개발을 위해 임시로 열어둔다
            ||  pageId.indexOf("getAlbumList.do") != -1 ||  pageId.indexOf("getAlbum.do") != -1 
            ||  pageId.indexOf("setAlbum.do") != -1 ||  pageId.indexOf("modifyAlbum.do") != -1 
            ||  pageId.indexOf("deleteAlbum.do") != -1 ||  pageId.indexOf("getContent.do") != -1 
            ||  pageId.indexOf("getCommentList.do") != -1 ||  pageId.indexOf("setComment.do") != -1 
            ||  pageId.indexOf("modifyComment.do") != -1 ||  pageId.indexOf("deleteComment.do") != -1 
            ||  pageId.indexOf("shareLink.do") != -1 ||  pageId.indexOf("getListenAlbum.do") != -1 
            ||  pageId.indexOf("deleteListenAlbum.do") != -1 ||  pageId.indexOf("getMyAlbumList.do") != -1 
            ||  pageId.indexOf("getMyAlbumCommentList.do") != -1 ||  pageId.indexOf("recommendAlbum.do") != -1) {  //얘만 예외로 풀어준다.  
            view = PageManager.callController(pageContext, session, request, response);
        }
    }

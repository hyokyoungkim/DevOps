...
function createForm() { //현재 페이지 값에 따라 양식이 바뀜. 기본값은 1=1Page
    let textForm = '';
    let textImg = '';
    ...
    if (form_value === 1) {
        ...
        <div class="relative">
                                ...
                                <div class="flex p-3 mt-5">
                                    ...
                                    <div class="w-1/3 border h-44" id="thumbnailImg"></div>
                                </div>
                                <div class="p-3 mt-5">
...
<div class="category border-2 pr-5 pl-5 pt-1 pb-1 inline-block rounded-sm m-1 cursor-pointer false" data-category="기타">
    ...
    if (form_value === 2 && formParams.chapter.length === 0) {
...
<div class="relative">
    ...
    <button class="float-right mt-10 mb-3 mr-5"  id="addContentBtn">
        <img src="../image/AddFilled.png" alt="플러스아이콘" class="inline-block">챕터 추가하기
    </button>
...
<div class="flex justify-center gap-2 mt-24">
    <button class="p-2 text-white rounded Button_normalSize__custom Button_subColor__custom" id="preBtn">이전단계로
        이동</button>
    <button class="p-2 text-white rounded Button_normalSize__custom Button_subColor__custom" id="nextBtn">저장 후 다음
        이동</button>
</div>`
    }
    if (form_value === 2 && formParams.chapter.length > 0) {
        console.log(formParams.chapter)
        textImg += `
        <li class="mt-3 mb-3"><img src="../image/CheckCircleOutlineFilledPurple.png" alt="" class="inline">앨범 정보</li>
        <li class="mt-3 mb-3"><img src="../image/CheckCircleOutlineFilled.png" alt="" class="inline">콘텐츠 입력</li>
        <li class="mt-3 mb-3"><img src="../image/CheckCircleOutlineFilled.png" alt="" class="inline">동의 및 발행</li>
        `
            ...
            <div class="content-details">`
                if(content.type === '1'){
                    ...
                    <div class="flex gap-2 pt-1 mt-1 mb-1">
                        <input type="text" class="w-full p-1 border" placeholder="동영상 페이지 URL을 입력해주세요" name="content" value="${content.content}">
                        <button class="p-2 text-white rounded Button_normalSize__custom Button_mainColor__purple">URL 확인</button>
                    </div>
                    ...
                    <div class="flex gap-2 pt-1 mt-1 mb-1">
                        <input type="file" class="w-full p-1 border" accept="application/pdf" onchange="handleFileUpload(event)">
                    </div>
                    <p class="mt-10 mb-10 leading-8">
                        · PDF 파일을 업로드 해주세요.
                    </p>
                    ...
                    <div class="flex gap-2 pt-1 mt-1 mb-1">
                        <input type="text" class="w-full p-1 border" placeholder="URL을 입력해주세요" name="content" value="${content.content}">
                    </div>
                    <p class="mt-10 mb-10 leading-8">
                        · 웹사이트의 URL을 입력해주세요.
                    </p>
                    ...
                    <div class="flex gap-2 pt-1 mt-1 mb-1">
                        <input type="file" class="w-full p-1 border" onchange="handleFileUpload(event)">
                    </div>
                    <p class="mt-10 mb-10 leading-8">
                        · 파일을 업로드 해주세요.
                    </p>
                    ...
                    <div class="wmde-markdown-var w-md-editor w-md-editor-show-live"
                            ...
                            name="" id="contentInput">${content.content}</textarea>
                                                </div>
                                            </div>
    ...
    if (form_value === 3) {
        ...
        <div class="Title_box__custom">동의 및 발행</div>
                            ...
                            <div class="flex justify-center gap-2 mt-24"><button
                                    ...
                                    이동</button><button
                                    class="p-2 text-white rounded Button_normalSize__custom Button_subColor__custom" id="submitBtn">Publish</button>
                            </div>
    ...
    styleCategory(); //카테고리 클릭했을 때 스타일 효과
}
createForm(); //최초 실행 시 나타날 페이지


//------------------------------------------1Page--------------------------------------------------//
function styleCategory() { //1Page 카테고리 스타일 효과(테두리)
    if (!(formParams.category === '')) { //카테고리를 클릭 후 2페이지->1페이지로 이동할 경우 스타일 적용
            const selectedCategory = formParams.category;
            const categoryDivs = document.querySelectorAll('.category');
            categoryDivs.forEach(div => {
            const category = div.getAttribute('data-category'); //data-카테고리의 값 가져오기
            if (category === selectedCategory) { 
                div.classList.add('border-[#9333ea]'); // 선택된 category와 일치하는 div에 테두리 색상 추가
            }
        });
    }
}
...
const radioPdf = document.getElementById('radioPdf');
...
function setCategory(e) { //1Page 카테고리 선택할 때 테두리 효과 적용
    const clickedDiv = e.currentTarget; 

    const categoryValue = document.querySelectorAll('#category + div > div'); // category h3 태그 아래의 모든 div 요소를 선택
    categoryValue.forEach(category => { 
        category.classList.remove('border-[#9333ea]'); //전체 효과 삭제
    });
    clickedDiv.classList.add('border-[#9333ea]');    // 현재 클릭된 div에 테두리 효과
    
    formParams.category = clickedDiv.textContent.trim();  // 클릭된 div의 텍스트 내용으로 formParams의 category 값을 업데이트
}

function clickCategory() { //1Page 카테고리 클릭 이벤트
    const categoryValue = document.querySelectorAll('#category + div > div'); //category 바로 다음에 있는 div 내의 모든 div 요소 선택

    categoryValue.forEach(category => { // 클릭 이벤트 리스너를 한 번만 추가
        category.removeEventListener('click', setCategory);
...
function fileUpload(formData){ //파일 업로드 (썸네일)
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if(form_value === 1){
                displayThumbnail(URL.createObjectURL(formData.get('file'))); // 수정된 부분/
...
function addThumbnail(e) { //1Page 썸네일추가 후 업로드
...
function displayThumbnail(imageUrl) { //1Page 썸네일 선택한 이미지를 미리보기로 보여줌
...
document.addEventListener('click', function (e) { //챕터/콘텐츠 클릭 이벤트
...
function updateChapterTitle(e) { //챕터 제목 실시간 반영
...
function updateFilePath(fileInput, fileUrl) { // 파일 경로 업데이트 및 formParams 업데이트 함수
...
function handleFileUpload(event) { //파일 업로드
...
function updateFormParamsWithFilePath(fileInput, uploadedFilePath) { // formParams의 파일 경로 업데이트 함수
...
function markdownContent(chapterIndex, contentIndex) { //type : text ->마크다운

    const contentInputId = `contentInput`;
    const contentInput = document.getElementById(contentInputId); //마크다운 입력 필드
    const italicIcon = document.getElementById('italicIcon'); //기울임
    const slashIcon = document.getElementById('slashIcon'); //코드
    const colonIcon = document.getElementById('colonIcon'); //인용
    const titleIcon = document.getElementById('titleIcon'); //제목
    const titleGroupIcon = document.getElementById('titleGroupIcon'); 
    const previewArea = document.getElementById('previewArea'); //미리보기 필드
    ...
    titleGroupIcon.querySelectorAll('button').forEach(button => {
        button.addEventListener('click', (event) => {
            const titleId = event.currentTarget.getAttribute('data-name'); // 'title1', 'title2' 등의 제목 레벨
            const level = parseInt(titleId.replace('title', ''), 10); // 제목 레벨 추출
            applyStyle(`title${level}`); // 해당 제목 레벨로 스타일 적용
            titleGroupIcon.style.display = 'none';  // 제목 그룹 다시 숨기기
        });
    });

    function applyStyle(style) { // 선택된 텍스트에 스타일 적용 함수
        const start = contentInput.selectionStart;
        const end = contentInput.selectionEnd;
        const content = contentInput.value;
        const selectedText = content.substring(start, end);

        let newContent; // 스타일에 따라 선택된 텍스트에 마크다운 문법 추가
        if (style === 'italic') {
        ...
        contentInput.value = newContent; // 텍스트 필드에 새로운 텍스트 적용 및 선택 범위 업데이트
        contentInput.setSelectionRange(start, start + newContent.length - content.length + selectedText.length);
        updatePreview(); // 미리보기 업데이트
    }

    function updatePreview() { // 미리보기 및 formParams 데이터 업데이트 함수
        const markdownContent = contentInput.value; // 입력된 마크다운 내용
        previewArea.innerHTML = marked(markdownContent); // 마크다운을 HTML로 변환하여 미리보기 영역에 표시

        // 마크다운 콘텐츠를 formParams에 업데이트
    ...
    contentInput.addEventListener('input', updatePreview); // 입력 필드에 변화가 있을 때마다 미리보기 업데이트
}
...
document.addEventListener('DOMContentLoaded', () => {  //페이지가 로드된 후 이벤트 리스너 추가
    localStorage.clear(); //로컬 스토리지 값 비우기
...
function createAlbum(albumData) { //앨범 만들기
    $.ajax({
        type: 'POST',
        url: '/codeenator/api/album/setAlbum.do?user_agency=web',
        ...
        success: function (data) {
            ...
            if(data.result === "success"){
                alert('앨범이 생성되었습니다.')
                location.href = '/codeenator/page/albumList.do'; //앨범리스트로 이동
            } else {
                alert(data.msg);
            }
        },
...
function clickBtn(e) { //클릭이벤트

    saveCurrentPageData(); // 페이지 이동 전에 현재 페이지 데이터를 저장

    if (e.target.id === 'preBtn') {  
        form_value--; // 페이지값 감소
        restorePageData(); // 페이지 데이터를 복원
        console.log(`페이지 ${form_value}로 이동`, formParams);
        generateArray(); // 폼을 재생성하여 데이터를 반영
        createForm(); // 폼을 재생성하여 데이터를 반영

    } else if (e.target.id === 'nextBtn') {
        if (!(formParams.name === '' && formParams.introduction === '' && formParams.category === '')) {
        form_value++; // 페이지값 증가
        restorePageData(); // 페이지 데이터를 복원
        console.log(`페이지 ${form_value}로 이동`, formParams);
        generateArray(); // 폼을 재생성하여 데이터를 반영
    ...
    if (e.target.id === 'submitBtn') {
        if (!clickTermCheck()) {
            ...
            return; // termAll이 체크되지 않으면 createAlbum 실행 X
        }
...
function restorePageData() {
    // 기본값으로 초기화
    formParams = getDefaultFormParams();
    // 모든 페이지의 데이터를 복원
    for (let i = 1; i <= 3; i++) {
        let localParamsJson = localStorage.getItem(`page${i}`);
        if (localParamsJson) {
            let pageData = JSON.parse(localParamsJson);
            if (i === 2) {
                formParams.chapter = pageData.chapter || [];
            } else if (i === 3) {
            } else {
                formParams = {...formParams, ...pageData};
            }
        }
    }
}
    ...
    if (form_value === 2) { // 2페이지에서만 챕터 데이터 업데이트
        ...
        formParams.chapter = Array.from(chapters).map(chapterDiv => {
            ...
            const contents = Array.from(chapterDiv.querySelectorAll('.groupContent')).map(contentDiv => {
                ...
                const contentId = contentDiv.id; // 여기서 contentId를 가져옴

                let contentPath = '';

                if (['2', '4'].includes(contentType)) { // 파일 타입
                    contentPath = fileUrls[contentId] || ''; // fileUrls에서 파일 URL을 가져옴
                } else if (contentType === '5') { // Text type
                    contentPath = contentDiv.querySelector('#contentInput')?.value || '';
                } else if (contentType === '1'){ 
                    contentPath = contentDiv.querySelector('button[name="urlData"]')?.value || '';
                }else { 

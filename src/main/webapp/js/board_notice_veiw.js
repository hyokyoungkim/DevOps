...
function getNotice() {
        $.ajax({
        type: 'GET',
        url: `/codeenator/api/board/getNotice.do?user_agent=web&seq=${seq_value}`,
        dataType: 'json', 
        success: function (data) {
            createNotice(data.resultMap);
        },
        error: function (err) {
...
function createNotice(list) {
    ...
    let textList = `
    <div class="m-3 mt-10 border-b "><b class="text-lg">${list.title}</b> <br>
    ...
    if(list.file){
        ...
        console.log(file);  // file 정보 콘솔 확인
        textList += `
        <div>
    ...
    if ((list.user_name === user_name || admin === 1)&& user ==! 0) {
...
function deletePage(seq) {
        ...
        $.ajax({
        type: 'POST',
        url: `/codeenator/api/board/deleteNotice.do?user_agency=web&seq=${seq}`,
        contentType: 'application/json',
        dataType: 'json', 
        success: function (data) {
            console.log(data);

...
function getQnaList() { //QnA 리스트 가져오기

    let url = `/codeenator/api/board/getQuestionList.do?user_agent=web&page=${page_value}&search_option=${search_value}`
...
function creteNoticeList(list) {
    ...
    <tr role="row">
        ...
        <td role="cell" class="pt-2 pb-2 border-b desktop:min-w-[50px] desktop:pr-5 desktop:pl-5 tablet:desktop:min-w-[50px] tablet:pl-5 tablet:pr-5">${noticeList[i].user_nickname}</td>
...
addQuestionBtn.addEventListener('click', function () {
    console.log(user);
    if (user === 1) {
        location.href = '/codeenator/page/board_qna_create.do';
    } else {
...
document.addEventListener('DOMContentLoaded', () => {
            ...
            listGroup.addEventListener('click', function (e) {
                ...
                if (target.tagName === 'TD' && target.hasAttribute('data-id')) { // 클릭된 요소가 td이고 data-id 속성이 존재하는 경우
                    ...
                    location.href = `/codeenator/page/board_qna_view.do?seq=${id_value}`;
                }
            });
